...
test('Vectors:scale', t => {
  ...
  const a3 = 6
  const s3 = scale(a3)
  t.equal(typeof s3, 'function', 'exported as a curried function');
  const v3 = { a: 1, b: 2 }
  t.deepEqual(s3(v3), { a: 6, b: 12 }, 'scales as a curried function')
...
test('Vectors:some', t => {
  ...
  t.equal(typeof somePositive, 'function', 'exported as a curried function')
  const v4 = { a: -3, b: -9, c: 6 }
  t.equal(somePositive(v4), true, 'matches as a curried function')
