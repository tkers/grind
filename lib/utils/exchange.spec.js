...
import { tryExchange, reduceExchanges, canAfford } from './exchange'
...
test('Exchange:canAfford', t => {

  t.equal(typeof canAfford, 'function', 'exported as a function');

  const i1 = { a: 2, b: 6 }
  const t1 = { a: -2, b: -5 }
  t.equal(canAfford(i1, t1), true, 'returns true if exchange can be processed')

  const i2 = { a: 2, b: 6 }
  const t2 = { a: -1, b: -7 }
  t.equal(canAfford(i2, t2), false, 'returns false if exchange is too expensive')
